...
async function produce() {
  try {
    ...
    await channel.assertQueue(SUBSCRIBED_USER_QUEUE_NAME, { durable: false }); // Declare a queue
    ...
    console.log(
      `[x] Sent ${ROUTING_KEY_FOR_SUBSCRIBED_USER}: '${JSON.stringify(messages)}'`,
    );
    channel.publish(
      EXCHANGE_NAME,
      ROUTING_KEY_FOR_NORMAL_USER,
    ...
    console.log(`[x] Sent ${ROUTING_KEY_FOR_NORMAL_USER}: '${JSON.stringify(messages)}'`);
